...
def main():
    ...
    if df is not None:
        ...
        all_months = sorted(
            ...
            key=lambda x: datetime.strptime(x, "%B %Y") if isinstance(x, str) else datetime.min,
            reverse=True,
        )
        ...
        if results:
            ...
            for idx, sample_type in enumerate(["sDNA", "cfDNA", "Blood", "RNA"]):
                with tabs[idx]:
                    ...
                    styled_df, raw_df = create_sample_type_analysis(results, sample_type)
                ...
                for idx, sample_type in enumerate(["sDNA", "cfDNA", "Blood", "RNA"]):
                    with cap_kit_tabs[idx]:
                        ...
                        if styled_cap_kit_dfs[sample_type] is not None:
                            st.dataframe(styled_cap_kit_dfs[sample_type], use_container_width=True)
                        else:
...
def create_consolidated_capturing_kit_analysis(df):
    ...
    if consolidated_results:
        ...
        def color_by_sample_type(row):
            ...
            return [f"background-color: {SAMPLE_TYPE_COLORS.get(sample_type, '#FFFFFF')}"] * len(row)
        
        # Apply styling
        styled_consolidated_df = consolidated_df.style.apply(color_by_sample_type, axis=1)
...
def main():
    ...
    if df is not None:
        ...
        if results:
            ...
            for idx, sample_type in enumerate(["sDNA", "cfDNA", "Blood", "RNA"]):
                with tabs[idx]:
                    st.markdown(f'<div class="big-header">{sample_type} Analysis</div>', unsafe_allow_html=True)
                    styled_df, raw_df = create_sample_type_analysis(results, sample_type)
                    st.dataframe(styled_df, use_container_width=True)

            # Capturing Kit Analysis
            with tabs[4]:
                st.markdown('<div class="big-header">Capturing Kit Analysis</div>', unsafe_allow_html=True)
                
                # Create tabs for the individual and consolidated views
                cap_kit_subtabs = st.tabs(["Consolidated View", "sDNA", "cfDNA", "Blood", "RNA"])
                ...
                with cap_kit_subtabs[0]:
                    st.markdown('<div class="big-header">Consolidated Capturing Kit Analysis</div>', unsafe_allow_html=True)
                    ...
                    styled_consolidated_df, _ = create_consolidated_capturing_kit_analysis(df)
                ...
                for idx, sample_type in enumerate(["sDNA", "cfDNA", "Blood", "RNA"]):
                    with cap_kit_subtabs[idx + 1]:
                        st.markdown(f'<div class="big-header">{sample_type} Capturing Kit Analysis</div>', unsafe_allow_html=True)
                        if styled_cap_kit_dfs[sample_type] is not None:
                            st.dataframe(styled_cap_kit_dfs[sample_type], use_container_width=True)
                        else:
                            st.info(f"No capturing kit data found for {sample_type}")
            ...
            # Show raw data option at the bottom
            if st.checkbox("Show Raw Data"):
                st.markdown('<div class="big-header">Raw Data</div>', unsafe_allow_html=True)
                st.dataframe(df)
